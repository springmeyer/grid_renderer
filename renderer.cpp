...
namespace agg
{
    ...
    //------------------------------------------------------------------------
    grid_rendering_buffer::grid_rendering_buffer(grid_value2* buf,
                                       unsigned width, 
                                       unsigned height,
                                       int      stride) :
        m_buf(0),
    ...
    //------------------------------------------------------------------------
    void grid_rendering_buffer::attach(grid_value2* buf,
                                  unsigned width, 
                                  unsigned height,
                                  int      stride)
    {
        ...
        {
            delete [] m_rows;
            m_rows = new grid_value2* [m_max_height = height];
        }

        grid_value2* row_ptr = m_buf;
    ...
    const int8u grid_rasterizer::s_default_gamma[] = 
    {
          0,  0,  1,  1,  2,  2,  3,  4,  4,  5,  5,  6,  7,  7,  8,  8,
          9, 10, 10, 11, 11, 12, 13, 13, 14, 14, 15, 16, 16, 17, 18, 18,
         19, 19, 20, 21, 21, 22, 22, 23, 24, 24, 25, 25, 26, 27, 27, 28,
    ...
    void grid_rasterizer::gamma(double g)
    {
        unsigned i;
        for(i = 0; i < 256; i++)
        {
    ...
    //------------------------------------------------------------------------
    void grid_rasterizer::gamma(const int8u* g)
    {
        memcpy(m_gamma, g, sizeof(m_gamma));
    }
    ...
    //------------------------------------------------------------------------
    bool grid_rasterizer::hit_test(int tx, int ty)
    {
        const cell* const* cells = m_outline.cells();
        if(m_outline.num_cells() == 0) return false;
